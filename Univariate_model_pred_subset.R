...
ComparMNAR_Univariate <- function(Xtrue,a=NULL,b=NULL,r,noise,Ns,modmecha,mecha,nbsim){
  ...
  results.list = foreach (ksim = 1:nbsim, .combine = "rbind")  %dopar% {
    ...
    while(!conv){
      ...
      if (conv2 & cov4){ conv=TRUE } else (print("Pas de convergence"))
    ...
    msePred=sapply(list(ThetabisNew[,1:p]*(1-M)), FUN=MSE, X2=X*(1-M))
    mseTrue=sapply(list(ThetabisNew[,1:p]), FUN=MSE, X2=Xtrue)
